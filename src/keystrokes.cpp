...
int
KeyStrokes::calcKeyWidth(unsigned code) const
{
...
void
KeyStrokes::paintKey(QPainter *painter, QPoint *point, KeyInfo *keyinfo)
{
	QRect rect;
	...
	painter->fillRect(rect.adjusted(3,3,-3,-3),
	                  QBrush(keyinfo->down ? Qt::darkGray : Qt::lightGray));
	painter->setPen(QPen(keyinfo->down ? Qt::white : Qt::black));
	painter->drawText(rect, Qt::AlignHCenter | Qt::AlignVCenter,
	                  keysyms[keyinfo->code].name);
	*point += QPoint(rect.width(), 0);
}


void
KeyStrokes::paintEvent(QPaintEvent *)
{
...
void
KeyStrokes::downKey(unsigned code)
{
...
void
KeyStrokes::upKey(unsigned code)
{
